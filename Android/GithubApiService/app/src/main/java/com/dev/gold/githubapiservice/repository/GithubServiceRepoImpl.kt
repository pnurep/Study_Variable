...
import com.dev.gold.githubapiservice.http.RetrofitClient
...
class GithubServiceRepoImpl : GithubServiceRepository {
    ...
    override fun getGithubRepo(): Observable<RepoData> {
        return RetrofitClient.githubRepoApi.getRepoInfo(
            "googlesamples", "android-architecture-components"
        )
    }

    override fun getStargazers(page: Int): Observable<List<Stargazer>> {
        return RetrofitClient.stargazerApi.getStargazers(
            "googlesamples", "android-architecture-components", page
        )
    }
